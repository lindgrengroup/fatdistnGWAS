...
/apps/well/bolt-lmm/2.3/bolt \
    ...
    --numThreads=10 \
    --statsFile=/path/to/$pheno/ukbb.fat-distn.$pheno.bolt.association.$sex.$grm.$ldsc.stats.grm.infinitesimal.txt \
    --bgenFile=/path/to/ukbiobank/data/ukb_imp_chr{1:22}_v2.bgen \
    --bgenMinMAF=1e-4 \
    --bgenMinINFO=0.3 \
    --sampleFile=/path/to/sample_names.txt \
    --statsFileBgenSnps=/path/to/$pheno/ukbb.fat-distn.$pheno.bolt.association.$sex.$grm.$ldsc.stats.bgen.infinitesimal.txt \
    --verboseStats
